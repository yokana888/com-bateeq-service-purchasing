...
namespace Com.DanLiris.Service.Purchasing.Test.Controllers.GarmentReports
{
	public class MonitoringCentralBillReceptionControllerTest
	{
		...
		public void Should_Error_Get_Report_Data()
		{
			...
			mockFacade.Setup(x => x.GetMonitoringTerimaBonPusatReport(null, null, 1, 25, "{}", 7))
		...
		public void Should_Error_Get_Report_Xls_Data()
		{
			...
			controller.ControllerContext.HttpContext.Request.Headers["x-timezone-offset"] = "0";
			var response = controller.GetXls(null, null, 1, 25, "{}");
			Assert.Equal((int)HttpStatusCode.InternalServerError, GetStatusCode(response));
		}
		[Fact]
		...
		{
			var mockFacade = new Mock<IMonitoringCentralBillReceptionFacade>();
			mockFacade.Setup(x => x.GetMonitoringTerimaBonPusatByUserReport(null, null, 1, 25, "{}", 7))
				.Returns(Tuple.Create(new List<MonitoringCentralBillReceptionViewModel> { ViewModel }, 25));
			...
			controller.ControllerContext.HttpContext.Request.Headers["x-timezone-offset"] = "0";
			var response = controller.GetReportByUser(null, null, null, 1, 25, "{}");
			Assert.Equal((int)HttpStatusCode.InternalServerError, GetStatusCode(response));
		}
		...
		public void Should_Error_Get_Report_By_User_Xls_Data()
		{
			...
			controller.ControllerContext.HttpContext.Request.Headers["x-timezone-offset"] = "0";
			var response = controller.GetXlsByUser(null, null, null, 1, 25, "{}");
			Assert.Equal((int)HttpStatusCode.InternalServerError, GetStatusCode(response));
		}
	}
}
