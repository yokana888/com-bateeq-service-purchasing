	...
	public class MonitoringCentralBillReceptionController : Controller
	{
		...
		public IActionResult GetReport(DateTime? dateFrom, DateTime? dateTo, int page, int size, string Order = "{}")
		{
			...
			try
			{
				...
				var data = facade.GetMonitoringTerimaBonPusatReport(dateFrom, dateTo, page, size, Order, offset);
		...
		public IActionResult GetReportByUser(string username, DateTime? dateFrom, DateTime? dateTo, int page, int size, string Order = "{}")
		{
			...
			try
			{
				...
				var data = facade.GetMonitoringTerimaBonPusatByUserReport(dateFrom, dateTo, page, size, Order, offset);
		...
		public IActionResult GetXls(DateTime? dateFrom, DateTime? dateTo, int page, int size, string Order = "{}")
		{
			try
			{
				...
				int offset = Convert.ToInt32(Request.Headers["x-timezone-offset"]);
				var xls = facade.GenerateExcelMonitoringTerimaBonPusat(dateFrom, dateTo, page, size, Order, offset);

				string filename = "Monitoring Peneriman Bon Pusat";
		...
		public IActionResult GetXlsByUser(string username, DateTime? dateFrom, DateTime? dateTo, int page, int size, string Order = "{}")
		{
			try
			{
				...
				int offset = Convert.ToInt32(Request.Headers["x-timezone-offset"]);
				var xls = facade.GenerateExcelMonitoringTerimaBonPusatByUser(dateFrom, dateTo, page, size, Order, offset);
